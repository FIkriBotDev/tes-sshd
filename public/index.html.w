...
<head>
  ...
  <style>
    ...
    .card-custom { max-width: 1400px; margin: 50px auto; padding: 20px; border-radius: 20px; box-shadow: 0 8px 20px rgba(0, 0, 0, 0.1); background: #fff; }
...
<body>
  ...
  <div class="chatbot-container" id="chatbotContainer">
    <iframe src="https://chatbot.exoduscloud.my.id" class="chatbot-iframe"></iframe>
  </div>
      ...
      try {
        ...
        Object.entries(rincian).forEach(([key, value]) => {
          if (!Array.isArray(value)) {
            ...
            li.innerHTML = `${key.replace(/([A-Z])/g, ' $1')}: <span class="badge-value">${value}</span>`;
...
// Kirim ke iframe chatbot
const chatbotIframe = document.querySelector("iframe.chatbot-iframe");
if (chatbotIframe) {
    chatbotIframe.contentWindow.postMessage({
        type: "nutrisi_data",
        payload: hasilJSON  // ← JSON dari NutriScan AI
    }, "*");  // gunakan domain tertentu untuk security
}
