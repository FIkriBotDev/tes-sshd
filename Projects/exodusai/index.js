    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        if (m.message.imageMessage || m.message.videoMessage || m.message.audioMessage || m.message.documentMessage) {
            try {
                ...
                // Ganti footer Pollinations dengan versi ExodusAI
                aiResponse = aiResponse.replace(
                    /---.*?(Pollinations\.AI).*?\[Dukung misi kami\]\(https:\/\/pollinations\.ai\/redirect\/kofi\).*?/gis,
                    '--- Ditenagai oleh ExodusAI. [Dukung misi kami](https://sociabuzz.com/exodusai) untuk menjaga agar AI tetap dapat diakses untuk semua orang.'
                );
