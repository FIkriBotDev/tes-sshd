    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        if (m.message.imageMessage || m.message.videoMessage || m.message.audioMessage || m.message.documentMessage) {
            try {
                ...
                aiResponse = aiResponse.replace(/[-–—]*\s*(Powered|Ditenagai)\s+oleh\s+Pollinations\.AI.*?(https:\/\/pollinations\.ai\/redirect\/kofi)?/gis, 'Ditenagai oleh ExodusAI. Dukung kami di https://sociabuzz.com/exodusai');
