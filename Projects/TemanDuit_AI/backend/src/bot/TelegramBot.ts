...
import { AIEngine, aiEngine } from '../engines/AIEngine';
...
export class TelegramBot {
  ...
  setupHandlers(): void {
    ...
    this.bot.command('start', async (ctx) => {
      ...
      if (existingUser) {
        await ctx.reply(
          ...
          `Ketik pesan apapun untuk mencatat keuanganmu, atau gunakan perintah berikut:\n\n` +
          `/saldo - Lihat saldo\n` +
      ...
      await ctx.reply(
        ...
        `TemanDuit adalah asisten keuangan pribadi berbasis AI yang membantu kamu:\n` +
    ...
    this.bot.command('hutang', async (ctx) => {
      ...
      const debts = this.debtEngine.getDebts(user.id).filter((d) => d.status !== 'PAID' && d.status !== 'CANCELLED');
      ...
      for (const debt of debts.slice(0, 10)) {
        const statusEmoji = { ACTIVE: '🟡', PARTIAL: '🟠', OVERDUE: '🔴', PAID: '✅', CANCELLED: '⚫' }[debt.status];
        msg += `${statusEmoji} *${debt.counterpartyName}*\n`;
    ...
    this.bot.command('tabung', async (ctx) => {
      ...
      try {
        await this.savingEngine.recordSaving(user.id, { action: 'saving', type: 'deposit', amount, description });
    ...
    this.bot.command('bantuan', async (ctx) => {
      await ctx.reply(
        `🤖 *Panduan TemanDuit*\n\n` +
        `*Catat Pengeluaran (Natural Language):*\n` +
        `• "beli bakso 15 ribu"\n` +
        `• "isi bensin 50rb"\n` +
        `• "bayar listrik 200k"\n` +
        `• "jajan kopi 25000"\n\n` +
      ...
      try {
        ...
        switch (parsed.action) {
          case 'expense': {
            ...
            const reply = await aiEngine.formatTransactionReply('expense', parsed.amount, parsed.description, updatedUser.balance);
          ...
          case 'income': {
            ...
            const reply = await aiEngine.formatTransactionReply('income', parsed.amount, parsed.description, updatedUser.balance);
            await ctx.reply(reply, { parse_mode: 'Markdown' });
            break;
          }
          ...
          case 'saving': {
            const tx = await this.savingEngine.recordSaving(user.id, parsed);
            const updatedUser = this.authService.getUserById(user.id)!;
            ...
            await ctx.reply(
              ...
              `📝 ${parsed.description}\n` +
              `🏦 Total tabungan: ${formatCurrency(updatedUser.savingBalance)}\n` +
              `💳 Saldo: ${formatCurrency(updatedUser.balance)}`,
              { parse_mode: 'Markdown' },
            );
            break;
          }
