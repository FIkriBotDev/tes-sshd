...
// === Util: Ambil teks dari .docx
const extractDocxText = async (filePath) => {
  const result = await mammoth.extractRawText({ path: filePath });
  return result.value;
};

// === Util: Eksekusi kode Python yang diberikan AI
const runPythonScript = (code, inputPath, callback) => {
  ...
  const finalCode = code
    ...
    .replace(/dokumen_path\s*=.*\n/, ''); // Hapus assignment ke 'dokumen_path'
...
// === Utility: Hapus blok ```python
const cleanCodeBlock = (code = "") =>
  code.replace(/^```(python)?\n/, "").replace(/```$/, "").trim();

// === Request ke RTIST API (fallback dual mode)
...
// === Endpoint: Edit dokumen dari URL
...
// === Endpoint: Buat dokumen baru dari prompt
app.get("/api/buat", async (req, res) => {
  try {
    ...
    fs.writeFileSync(dummyInput, Buffer.from([])); // dummy docx kosong
