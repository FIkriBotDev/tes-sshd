...
const runPythonScript = (code, inputPath, callback) => {
  ...
  // Bersihkan baris dokumen_path dan ubah doc.save(...)
  let finalCode = code
    .replace(/dokumen_path\s*=.*\n?/g, '') // Hapus deklarasi dokumen_path
    .replace(/Document\(['"](.+?)['"]\)/, `Document("${inputPath}")`) // input path
    .replace(/doc\.save\((.*?)\)/g, `doc.save("${outputDocx}")`); // save output
