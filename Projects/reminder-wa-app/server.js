...
app.get('/dashboard', requireLogin, async (req, res) => {
  ...
  const userReminders = reminders.filter(r => r.userId === req.session.user.id);
  const tz = "Asia/Jakarta";
  res.render('dashboard', { reminders: userReminders, tz, user: req.session.user });
});

// ------------ Reminder CRUD (create/edit/delete) ------------
...
// ------------ Settings (update username / phone) ------------
app.get('/settings', requireLogin, async (req, res) => {
  const users = await readDB(USERS_DB);
  const user = users.find(u => u.id === req.session.user.id);
  // reuse register view for editing
...
async function schedulerTick() {
  ...
  ? `Halo ${rem.username || ''} 👋\n\n🔔 REMINDER! 🔔\n\nSekarang sudah jam ${hhmm} waktunya ${rem.title || '-'}.\n\n✨ Kegiatan: ${rem.title || '-'}\n📅 Tanggal: ${dateStr}\n⏰ Waktu: ${hhmm}\n📍 Lokasi: ${rem.location || '-'}\n\n💡 Catatan: ${rem.description || '-'}`
  : '';
